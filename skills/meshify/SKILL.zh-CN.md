...
拿到模型
  ...
  ├─ 是 STEP？ → meshify doctor 先确认 Tier1；未装按指引装（见 references/zh-CN/cad-step.md）
  ├─ 太大/面数高？
...
**读法**：`metrics.face_reduction/byte_reduction` 看效果；`warnings[].code` 看降级（不是失败）；`errors` 非空即失败。警告码全表见 references/zh-CN/troubleshooting.md。
...
- 环境问题（WASM 加载失败 / uv 未装 / 磁盘不足）→ `meshify doctor`，或 references/zh-CN/troubleshooting.md
- STEP 转换失败/精度调整 → references/zh-CN/cad-step.md
- 各命令参数细节 → references/zh-CN/{simplify,segment,texture,convert,optimize}.md
- 双内核差异与一致性 → references/zh-CN/tiering.md；宿主兼容 → references/zh-CN/support-matrix.md
- manifest 字段级文档 → references/zh-CN/report-schema.md
